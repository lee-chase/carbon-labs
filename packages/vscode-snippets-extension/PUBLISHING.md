...
**Important Note on Publisher Names:**

The publisher name differs between the two marketplaces due to account linking
requirements:

- **Open VSX**: Uses `lee-chase` (linked to GitHub account)
- **VSCode Marketplace**: Uses `LeeChase` (auto-generated by VSCode Marketplace)

Until these publisher names match, you must build the `.vsix` file twice:

1. Build with `"publisher": "lee-chase"` in `package.json` for Open VSX
2. Change to `"publisher": "LeeChase"` and rebuild for VSCode Marketplace

**Note:** Changing the publisher name in VSCode Marketplace would make it appear
as a new extension, so maintaining separate builds is currently necessary.

To publish to both VSCode Marketplace and Open VSX:

```bash
# Build for Open VSX (publisher: "lee-chase")
yarn build
yarn package
...
# Update package.json publisher to "LeeChase"
# Then rebuild for VSCode Marketplace
yarn build
yarn package
